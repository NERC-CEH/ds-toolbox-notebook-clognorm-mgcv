...
Note that although we pass a matrix to `mgcv` as the response, we only have a univariate response variable, the second column is only for indicating censoring to the code.
...
We can add a confidence band around this plot by manually calculating the upper and lower bounds, but `mgcvUtils` also provides a function to do this for you in an analogous way to the [`gratia`](https://gavinsimpson.github.io/gratia/) package, via the `fitted_values_clognorm` function^[Note that the `fitted_values` function in `gratia` will not work with the `clognorm` family at this time.].
